...
func assertPublishedHookEnvelopes(t *testing.T, run hookRunner, pack string) {
	...
	for _, testCase := range []struct {
		...
		{harness: "codex", plain: true},
		{harness: "kiro", plain: true},
		{harness: "copilot", want: map[string]any{"additionalContext": pack}},
		{harness: "gemini", input: `{"hook_event_name":"BeforeAgent"}`, want: map[string]any{
			"hookSpecificOutput": map[string]any{"hookEventName": "BeforeAgent", "additionalContext": pack},
		}},
		{harness: "devin", input: `{"hook_event_name":"UserPromptSubmit"}`, want: map[string]any{
			"hookSpecificOutput": map[string]any{"hookEventName": "UserPromptSubmit", "additionalContext": pack},
		}},
		{harness: "cursor", want: map[string]any{"additional_context": pack}},
		{harness: "antigravity", input: `{"invocationNum":0}`, want: map[string]any{
			"injectSteps": []any{map[string]any{"ephemeralMessage": pack}},
		}},
		{harness: "cline", input: `{"taskId":"matrix-first"}`, want: map[string]any{
			"cancel": false, "contextModification": pack,
		}},
	} {
		t.Run(testCase.harness, func(t *testing.T) {
			...
			if testCase.plain {
				if output != pack+"\n" {
					t.Fatalf("plain output = %q, want %q", output, pack+"\n")
				}
				return
			}
...
func assertEmptyHookEnvelopes(t *testing.T, run hookRunner) {
	t.Helper()
	for _, harness := range []string{"claude", "codex", "kiro", "copilot", "gemini", "devin", "cursor", "antigravity", "cline"} {
...
func assertHookEventGates(t *testing.T, run hookRunner, pack string) {
	...
	if err != nil || !reflect.DeepEqual(decodeHookEnvelope(t, output), map[string]any{"cancel": false, "contextModification": pack}) {
