...
- **Stored reads and MCP make no network request; declared CLIs may.** During `fkf sync`, or an explicit trust-gated `fkf read --body`, a provider CLI can talk to its provider under its own credentials and audit logs. `context`, `find`, `graph`, MCP, and `read` without `--body` touch only files.
