...
```text
fkf: invalid configuration: ~/brain/fkf.yaml: yaml: unmarshal errors:
  line 7: field secrets not found in type core.fileConfig
```
...
1. No non-comment line of Go mentions `TOKEN`, `SECRET`, `PASSWORD`, `API_KEY`, or `PAT`.
...
- **Only explicit command surfaces execute what the base declares:** source `auth:` and `run:` during `fkf sync` (including the write-free `--preview`), source hooks during `fkf test`, record `body:` during `fkf read --body`, and bounded `auth:` probes during `fkf brief` or explicit `fkf status --live`. Brief and live status never collect evidence. MCP and ordinary stored reads never execute a declared command. `?jq=` is evaluated in-process by gojq with environment and file-loading builtins disabled. `fkf trust --check` reports the state and records nothing, while `--all` prints the full disclosure even when a concise change report is available.
...
`auth:` takes no placeholder at all. It is literal argv, runs once per sync and only for a source that has due work, and its stdout and stderr are discarded rather than logged.
...
Reads are bounded: 1 MiB for configuration, body manifests, and each trusted file under `bin/` or `tests/`; 4 MiB for a Markdown page or cached body; 64 MiB for a collected document; and 64 MiB per captured subprocess stream. The rebuildable body cache is additionally capped at 4,096 entries and 512 MiB. A command that overruns its stream limit fails its day rather than being truncated into a document that parses and lies.

`fkf status` diagnoses unsafe filesystem modes and prints a precise owner-only repair command; it never changes the base. The command excludes `.git`, sets base directories to `0700` and ordinary files to `0600`, and preserves executable intent under both `bin/` and `tests/` while removing group and other access:
...
The omitted tail applies `0600` to files outside the two execution trees, then walks every file below `bin/` and `tests/` and applies `0700` only when that file is already executable, otherwise `0600`. `fkf status` prints the full directly runnable command; the shortened form above only keeps this page readable.
...
- **Trust answers "did fkf's execution definition change", not "is this safe".** The gate proves you read the command definitions and hashed `bin/` and `tests/` trees and that they have not changed since. It does not sandbox them: a trusted command runs with your privileges and can deliberately interpret any local file. Authored and collected layers are data, not part of the digest; sourcing one from a command turns mutable data into code outside the guarantee.
...
- **Stored reads and MCP make no network request; explicit execution paths may.** During `fkf sync`, an explicit trust-gated `fkf read --body`, or an `auth:` probe from `fkf brief` or `fkf status --live`, a provider CLI can talk to its provider under its own credentials and audit logs. Brief and live status discard probe output and collect nothing. Sources with `bodies: sync` prefetch only after their evidence write; failures remain visible and retryable. `bodies: cache` writes only after an explicit body read, while the default `none` never stores the text. An explicit `fkf upgrade` asks `curl` for fixed GitHub release URLs and touches no base. `context`, `find`, `graph`, MCP, and `read` without `--body` touch only files; cached bodies remain untrusted data.
