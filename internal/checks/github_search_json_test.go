...
type githubSearchRecord struct {
	URL       string   `json:"url"`
	Assignees []string `json:"assignee_uris"`
}
...
func githubSearchAPIItem(url string) map[string]any {
	return map[string]any{
		...
		"assignees":      []any{map[string]any{"login": "owner"}},
	}
}
...
printf '%s\n' "$*" >> "$GH_CALL_LOG"
case "$*" in
  *"q=is:pr assignee:@me updated:2026-05-04T00:00:00Z..2026-05-04T00:00:03Z"*) file=root.json ;;
  *"q=is:pr assignee:@me updated:2026-05-04T00:00:00Z..2026-05-04T00:00:01Z"*) file=left.json ;;
  *"q=is:pr assignee:@me updated:2026-05-04T00:00:02Z..2026-05-04T00:00:03Z"*) file=right.json ;;
...
`)
	...
	command := githubSearchCommand(t, fakeBin, fixtures, calls,
		"prs", "assignee", "2026-05-04T00:00:00Z", "2026-05-04T00:00:04Z")
	...
	for _, record := range records {
		...
		if !slices.Equal(record.Assignees, []string{"actor:github.com/owner"}) {
			t.Fatalf("assignees for %s = %v, want the projected owner identity", record.URL, record.Assignees)
		}
	}
	...
	for _, expected := range []string{
		"q=is:pr assignee:@me updated:2026-05-04T00:00:00Z..2026-05-04T00:00:03Z",
		"q=is:pr assignee:@me updated:2026-05-04T00:00:00Z..2026-05-04T00:00:01Z",
		"q=is:pr assignee:@me updated:2026-05-04T00:00:02Z..2026-05-04T00:00:03Z",
	} {
...
`)
	...
	if !strings.Contains(string(log), "q=is:issue assignee:@me updated:2026-05-04T00:00:00Z..2026-05-04T00:00:00Z") {
...
`)
	for _, test := range []struct {
		...
		{name: "assigned pull requests", kind: "prs", mode: "assignee", qualifier: "q=is:pr assignee:@me"},
		{name: "assigned issues", kind: "issues", mode: "assignee", qualifier: "q=is:issue assignee:@me"},
	} {
...
func TestGitHubSearchJSONRejectsTheObsoleteReviewedMode(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "expected prs assignee or issues assignee") {
...
func TestGitHubSearchSourcesUseTheCompleteCollector(t *testing.T) {
	want := map[string][]string{
		"github-pull-requests": {githubSearchHelper, "prs", "assignee", "{{start}}", "{{end}}"},
		"github-issues":        {githubSearchHelper, "issues", "assignee", "{{start}}", "{{end}}"},
	}
