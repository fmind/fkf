...
func TestAtuinHistoryOmitsDeletedRowsAndCommandText(t *testing.T) {
	...
	if len(records) != 1 || records[0].ID != "active" || records[0].Title != "git status" ||
		records[0].Tool != "git" || records[0].Action != "status" {
		t.Fatalf("atuin history records = %+v, want only the classified active row", records)
	}
...
INSERT INTO history VALUES (
  ...
  10, 0, 'git status --token=active-command-secret', '/workspace/active', NULL
);
INSERT INTO history VALUES (
...
func TestGWSCalendarsKeepsOnlyInWindowStartsAndProjectsMetadata(t *testing.T) {
	bin := t.TempDir()
	fakeGWS := `#!/bin/sh
set -eu
case "$*" in
*"calendarList list"*) cat <<'JSON'
{"items":[{"id":"engineering@example.test","summary":"Engineering","accessRole":"owner","primary":true,"timeZone":"Europe/Paris"}]}
JSON
;;
...
{"items":[
  ...
  {"id":"inside","status":"confirmed","eventType":"default","summary":"Inside","htmlLink":"https://calendar.example.test/event/inside","start":{"dateTime":"2026-05-04T11:00:00+02:00","timeZone":"Europe/Paris"},"end":{"dateTime":"2026-05-04T12:00:00+02:00"},"organizer":{"email":"organizer#calendar@example.test","displayName":"Private name"},"attendees":[{"email":"guest@example.test","responseStatus":"accepted","comment":"private response"}],"description":"sensitive-calendar-body","location":"private-location","conferenceData":{"conferenceId":"meet-safe-id","entryPoints":[{"uri":"https://meet.example.test/private"}]},"attachments":[{"fileId":"doc-1","fileUrl":"https://docs.google.com/document/d/doc-1/edit","title":"Inside - Notes by Gemini","privateField":"not-projected"}]},
...
`
	...
	script := filepath.Join(repositoryRoot(t), "presets", "bin", "gws-calendars-json.sh")
	...
	command.Env = append(os.Environ(), "PATH="+strings.Join([]string{bin, helperBin, os.Getenv("PATH")}, string(os.PathListSeparator)))
	output, err := command.CombinedOutput()
	if err != nil {
		t.Fatalf("gws-calendars-json.sh error = %v\n%s", err, output)
	}
	var records []map[string]any
	if err := json.Unmarshal(output, &records); err != nil {
		t.Fatalf("decode gws-calendars-json.sh output: %v\n%s", err, output)
	}
	if len(records) != 1 || records[0]["id"] != "inside" {
	...
	for _, wanted := range []string{
		`"conference_id":"meet-safe-id"`, `"fileId":"doc-1"`,
		`"attachment_uris":["document:drive.google.com/doc-1"]`,
	} {
		if !strings.Contains(string(output), wanted) {
			t.Errorf("calendar metadata projection omitted %q: %s", wanted, output)
		}
	}
	for _, private := range []string{"sensitive-calendar-body", "private-location", "private response", "meet.example", "privateField", "Private name"} {
		if strings.Contains(string(output), private) {
