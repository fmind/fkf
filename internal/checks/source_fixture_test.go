...
func TestEveryPresetRunBuildsDirectArgvAfterSubstitution(t *testing.T) {
	...
	for _, preset := range services.Presets {
		...
		for _, name := range config.SourceNames() {
			...
			command, err := sources.BuildRunCommand(source, sources.Environment{Root: root}, window, time.Minute)
			if err != nil {
				t.Fatal(err)
			}
...
func TestEveryGWSPageAllCollectorDeclaresAnExplicitPageLimit(t *testing.T) {
	...
	for _, path := range []string{
		...
		filepath.Join(repositoryRoot(t), "presets", "bin", "gws-calendars-json.sh"),
		filepath.Join(repositoryRoot(t), "presets", "bin", "gws-chat-messages.sh"),
...
func TestEveryShippedPresetArtifactBelongsToADeclaredSource(t *testing.T) {
	...
	fixtures, err := filepath.Glob(filepath.Join(repositoryRoot(t), "services", "testdata", "sources", "*.json"))
	if err != nil {
		t.Fatal(err)
	}
