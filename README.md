...
Set `FKF_INSTALL_DIR` to an absolute directory to change the destination, or `FKF_VERSION=v3.0.2` to pin a release. You can also download an archive and `checksums.txt` directly from the [latest release](https://github.com/fmind/fkf/releases/latest). Each archive is one installation unit containing the binary, license, README, and linked-dependency notices.
